...
export async function generateStaticParams() {
  return silverProducts.map((product) => ({
    id: product.id,
  }))
}

export default function SilverProductPage({ params }: { params: { id: string } }) {
  ...
  const relatedProducts = silverProducts
          ...
          <div className="space-y-6 animate-fade-in-up delay-100">
            ...
            <div className="space-y-4 text-stone-700">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium">92.5% Sterling Silver</span>
              </div>
              <div className="flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {relatedProducts.map((relatedProduct, index) => (
              <div key={relatedProduct.id} className={`animate-fade-in-up delay-${100 + index * 100}`}>
                <ProductCard {...relatedProduct} category="silver" />
              </div>
            ))}
          </div>
        </div>
